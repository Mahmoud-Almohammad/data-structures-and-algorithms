...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce( (acc, fam) => {
    if (fam.children) {
      return acc + fam.children.length;
    }
    return acc;
  } ,0);
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce( (acc, val) => {
    return statName === val.stat.name ? val : acc;
  } ,null);
};
...
const extractChildren = (arr) => {
  // Solution code here...
  const filtered = arr.filter(family => family.name.includes('a'));
  return filtered.reduce( (acc, character) => {
    if(character.children) {
      return acc.concat(character.children);
    }
    return acc;
  } ,[]);
};
