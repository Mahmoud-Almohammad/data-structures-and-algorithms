...
const longestString = (arr) => {
  ...
  const longest = arr.reduce( (a, b) => {
    a.length > b.length ? a : a = b;
    return a;
  }, '');
  const idx = arr.indexOf(longest);
  return idx;
};
...
const firstLetters = (arr) => {
  // Solution code here...
  const newArr = arr.map( str => str.substring(0,1));
  return newArr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const findHappiness = (arr) => {
  // Solution code here...
  const happy = arr.filter( str => str.includes(':)'));
  return happy;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  const digit = /\d/;
  const formatedNumbersArray = [];
  for(let i = 0; i < arr.length; i++) {
    let newFormat = '';
    for(let j = 0; j < arr[i].length; j++) {
      if(digit.test(arr[i][j])) {
        newFormat += arr[i][j];
      }
    }
    formatedNumbersArray.push(newFormat);
  }
  return formatedNumbersArray;
};
...
const allHappy = (arr) => {
  // Solution code here...
  const checkStrings =  arr.reduce( (acc, str) => {
    return str.includes(':)') ? acc += 'true' : acc += 'false';
  }, '');
  return checkStrings.includes('false') ? false : true;
};
