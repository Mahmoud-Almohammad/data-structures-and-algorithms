...
function returnTen(str){
  ...
  return arrStr.splice(arrStr.length-10);
}
...
const containsAnd = (arr) => {
  // Solution code here...
  return arr.filter( str => str.toLowerCase().includes('and'));
};
...
const filterStringsWithVowels = (arr) => {
  ...
  const newArr = arr.filter(str => {
    const vowels = /\b[^\W\d]*[aeiouAEIOU][^\W\d]*\b/;
    if (vowels.test(str)) {
      return true;
    }
  });
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter( int => minBaseStat < int.baseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter( int => minBaseStat < int.baseStat).map( obj => obj.stat.name);
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter( element => typeof element === 'number').map( num => {
    if ( num % 2 ) {
      return 'odd';
    }
    return 'even';
