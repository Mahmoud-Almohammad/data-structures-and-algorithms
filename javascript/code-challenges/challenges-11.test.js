...
const validateWord = (word) => {
  // Solution code here...
  const wordTetsing = /^\D{5,10}$/g;
  return wordTetsing.test(word);
};
...
const hasNumber = (string) => {
  // Solution code here...
  const testingEx = /[A-Za-z]+\d+/;
  return testingEx.test(string);
};
...
const validateEmail = (email) => {
  // Solution code here...
  const emailRegex = /^\w+(|[.]\w+)[@]\w+(.com|.net|.org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const numRegex = /^(\(\d{3}\)|\d{3})[ -]?\d{3}[ -]?\d{4}$/;
